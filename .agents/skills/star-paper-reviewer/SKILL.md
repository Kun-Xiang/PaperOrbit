...
description: >-
  ...
  documents to $star-metd-summarize. Use when the user invokes $star-paper-reviewer, wants
...
- The workflow has no mandatory gates — the target resolution may need one `ask_user_question` call when the argument is ambiguous; otherwise the skill runs to its report without questions, which is what makes it safe on a timer. In non-interactive `codex exec`, fall back to one concise plain-text question.
