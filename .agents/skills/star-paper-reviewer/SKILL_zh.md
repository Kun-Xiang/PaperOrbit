...
description: >-
  ...
  $star-metd-summarize。当用户调用 $star-paper-reviewer、想在投稿前审稿、想知道审稿人
...
- 工作流没有强制门——参数有歧义时目标解析可经 `ask_user_question` 问一次；否则一路跑到报告、不提问，这正是它可以挂定时任务的原因。非交互的 `codex exec` 中退回单个精炼的纯文本问题。
