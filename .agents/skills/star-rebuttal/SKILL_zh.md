...
description: >-
  ...
  的质检。只写 metds/rebuttal/<cycle>/。当用户调用 $star-rebuttal、收到审稿意见问怎么办、
...
2. **诊断底层疑虑，不停留在表面句子。** 每段评审拆成原子意见；每条意见给出其背后的疑虑、类别、严重度、共享度、决策影响与解决置信度（`triage_spec_zh.md` Part B–C）。含混的评论用意图诊断卡呈现；当解释会改变稀缺资源的去向时，问一次门控问题——绝不默默猜测。
...
把每份评审拆成原子意见（C1、C2、…），按 `triage_spec_zh.md` Part B–C 填 matrix：表面评论、底层疑虑、确有其事的替代解释、置信度、类别、回应模式、严重度、共享度、决策影响、解决置信度。真正含混的解释以意图诊断卡呈现——每张至多一次门控问题，选项即候选解释，且只在答案会改变要跑什么时才问。matrix 按节完成即写盘。
...
2. 从 `assets/experiment_ledger_template_zh.md` 写 `experiment_ledger.md`，用一次门控问题（P0/P1 行可多选，标注推荐）确认现在就跑的集合。
...
- 各道门经 `ask_user_question` 工具提问，一次一问——仅在非交互的 `codex exec` 中退回单个精炼的纯文本问题：可行性决定（Step 1）、真正含混的意图诊断卡（Step 2）、现在就跑的集合（Step 4）、草稿确认（Step 6）。任何跨门写入前需要明确答复。
