...
多种解释都说得通时，绝不默默选边。在 matrix §5 填卡：**评审原文**（忠实短引）；**最可能的底层疑虑** / **替代解释**；**置信度**（high / medium / low）与**不确定的原因**（缺引用、措辞含糊、分数与文字不符、自相矛盾）；**能同时回答两种解释的证据**——可行时优先；**问作者的问题**——仅当必须依赖作者知识时，经 `ask_user_question` 以候选解释作选项；**安全回应策略**：两种读法下都成立的措辞。
