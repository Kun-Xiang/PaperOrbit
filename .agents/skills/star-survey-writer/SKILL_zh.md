...
跑 2–3 个探索性查询（top 10–20，仅元数据）感知领域形状，然后填方案：检索关键词组、显现的细分方向、筛选标准（venue、年份窗、强制行）、目标广度（~100+；窄领域如实缩减）、目标读者。经 `ask_user_question` 确认——*批准 / 调整 / 收窄主题*——迭代到批准。锁定方案（`stage.proposal: done`）；之后的更改要显式重开。
...
按 `funnel_spec_zh.md` Part C 应用标准：先强制行，再按排名补足 15–25 池。经 `ask_user_question`（可多选，标注推荐；用户可增可删）呈现带一句话理由的池。确认的池与排除说明记入 `broad_scan.md`。
...
按 `outline_spec_zh.md` 从被扫描综述导出候选结构，合并成带每节论文指派与目标篇幅的大纲，跑覆盖检查（池内每篇有归属；每节 ≥2 篇支撑或明标），经 `ask_user_question` 确认。写 `outline.md`。
...
- 三道门经 `ask_user_question` 工具提问，一次一问——仅在非交互的 `codex exec` 中退回单个精炼的纯文本问题：方案（Step 1）、池（Step 3）、大纲（Step 5）——外加重生成时的标准覆写问题。门所覆盖的扫描或写入前需要明确批准。
