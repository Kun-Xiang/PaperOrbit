...
| 10 | Rebuttal draft stale | a `metds/rebuttal/<cycle>/experiment_ledger.md` names a run in its tracking table whose `wkdrs/<run>/EXPT_ANALYSIS_<date>.md` exists **and** (that cycle's `rebuttal.md` is absent **or** its `drafted:` is older than the newest such analysis date) | `/star-rebuttal integrate` |
| 11 | Paper stale | `paper/project_context.md` exists **and** its `sources:` map records a value for a source document that differs from that document's current state field (`generated:` / `updated:` / bib entry count) | `/star-paper-writer` |
| 12 | Paper review missing | `paper/main.tex` exists **and** at least one `paper/sections/*.tex` exists **and** no `wkdrs/reviews/paper_*.md` exists | `/star-paper-reviewer` |
| 13 | Survey stale | a `metds/survey/<slug>/survey.md` exists **and** its `sources:` records a `reference.bib` entry count or `refs_index.md` audit date that differs from the current one | `/star-survey-writer refine` |
...
2. **Debt on finished work** — a triggered coverage row on work that is already done, taken in order of how fast the debt compounds: backfill (row 9) → review (rows 3, 4) → analysis (row 5) → rebuttal (row 10) → aggregate (row 6) → summarize (rows 7, 8) → paper (row 11) → paper review (row 12) → survey (row 13) → refs (row 2). Row 9 leads because it is the debt that hides the others: until an adopted project's finished leaves carry `exec_status: done`, rows 3 and 5 cannot fire on them at all, and tier 3 will happily recommend executing a leaf whose work is already sitting on disk. Rebuttal (row 10) follows analysis because its deadline is external: once the analyses a cycle waited for exist, integrating them is the debt that expires fastest. Paper staleness (row 11) sits after summarize because the paper compiles from the documents those rows refresh. Every coverage row except row 1 is reachable here; row 1 is tier 4 because starting a new topic is not a debt. Refs comes last despite being early in the flow: a missing survey costs positioning at write-up time, while unreviewed code costs every leaf built on top of it — so "go read the literature" must never outrank "the run you just finished was never reviewed". Debt outranks progress because it compounds: every further leaf executed on unreviewed code, or quoted from a stale ledger, widens what has to be redone. The next leaf, by contrast, does not expire.
...
- a `*.md` directly inside one of the two registered non-run `wkdrs/` dirs, under a name §8 does not register there: in `wkdrs/reviews/` (the shared no-run fallback) the registered names are `code_<scope>_<date>.md`, `paper_<scope>_<date>.md`, and `<prefix>_<slug>_<date>.md` (numeric prefix); in a `wkdrs/env_<name>_<date>/` dir the registered name is `ENV_REPORT.md`. Any other `wkdrs/` subdir is audited as a run dir under the previous bullet;
