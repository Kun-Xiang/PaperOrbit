...
2. **Diagnose the underlying concern, not the surface sentence.** Every review paragraph is split into atomic concerns; every concern gets the doubt behind it, its class, severity, sharedness, decision impact, and resolution confidence (`triage_spec.md` Parts B–C). An ambiguous comment gets an Intent Diagnosis Card and, when the interpretation would redirect scarce resources, one AskUserQuestion — never a silent guess.
...
2. Write `experiment_ledger.md` from `assets/experiment_ledger_template.md` and confirm the run-now set via one AskUserQuestion (multiSelect over P0/P1 rows, recommendations marked).
...
- Gates go through AskUserQuestion, one question per call: the viability decision (Step 1), genuinely ambiguous Intent Diagnosis Cards (Step 2), the run-now set (Step 4), and the draft confirmation (Step 6). If it is unavailable (headless / scripted), fall back to plain text — still one at a time, still an explicit answer before any gate-crossing write.
