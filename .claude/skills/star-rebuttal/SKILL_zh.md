...
2. **诊断底层疑虑，不停留在表面句子。** 每段评审拆成原子意见；每条意见给出其背后的疑虑、类别、严重度、共享度、决策影响与解决置信度（`triage_spec_zh.md` Part B–C）。含混的评论用意图诊断卡呈现；当解释会改变稀缺资源的去向时，问一次 AskUserQuestion——绝不默默猜测。
...
把每份评审拆成原子意见（C1、C2、…），按 `triage_spec_zh.md` Part B–C 填 matrix：表面评论、底层疑虑、确有其事的替代解释、置信度、类别、回应模式、严重度、共享度、决策影响、解决置信度。真正含混的解释以意图诊断卡呈现——每张至多一次 AskUserQuestion，选项即候选解释，且只在答案会改变要跑什么时才问。matrix 按节完成即写盘。
...
2. 从 `assets/experiment_ledger_template_zh.md` 写 `experiment_ledger.md`，用一次 AskUserQuestion（multiSelect 于 P0/P1 行，标注推荐）确认现在就跑的集合。
...
- 各道门走 AskUserQuestion，一次一问：可行性决定（Step 1）、真正含混的意图诊断卡（Step 2）、现在就跑的集合（Step 4）、草稿确认（Step 6）。不可用（headless / 脚本化）时退回纯文本——仍一次一问，任何跨门写入前仍需明确答复。
