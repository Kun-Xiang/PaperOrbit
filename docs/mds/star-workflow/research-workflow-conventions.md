...
The rules every STAR research workflow skill follows. The seventeen skills — `star-proj-adopt`, `star-idea-storm`, `star-plan-coach`, `star-refs-reviewer`, `star-code-architect`, `star-env-builder`, `star-plan-decomposer`, `star-plan-executor`, `star-code-reviewer`, `star-expt-analyst`, `star-plan-reviser`, `star-flow-status`, `star-metd-summarize`, `star-rebuttal`, `star-paper-writer`, `star-paper-reviewer`, `star-survey-writer` — each carry their own workflow, write boundary, and rubric. What they share lives here, once.
...
**Skills that never commit** — git usage is read-only (`status` / `diff` / `log`): `star-flow-status`, `star-refs-reviewer`, `star-expt-analyst`, `star-metd-summarize`, `star-paper-reviewer`.
...
| `star-rebuttal` | offered once at session end | only `metds/rebuttal/<cycle>/` — the cycle this session created or edited |
| `star-paper-writer` | offered once at session end | only the `paper/**` files this session touched |
| `star-survey-writer` | offered once at session end | only `metds/survey/<slug>/` — the cycle this session created or edited |
...
| Rebuttal | `star-rebuttal` | `metds/rebuttal/<cycle>/` — `reviews_raw.md`, `concern_matrix.md`, `experiment_ledger.md`, `rebuttal.md`, `resubmission_plan.md` | matrix `stage:` map + `updated:`; rebuttal `drafted:` |
| Paper | `star-paper-writer` | `paper/project_context.md`, `outline.md`, `main.tex`, `sections/`, `REVISION_LOG.md` | `project_context.md` `updated:` + `sources:` |
| Paper review | `star-paper-reviewer` | `wkdrs/reviews/paper_<scope>_<date>.md` | date in filename |
| Survey | `star-survey-writer` | `metds/survey/<slug>/` — `proposal.md`, `broad_scan.md`, `outline.md`, `sections/`, `survey.md` | proposal `stage:` map; survey `generated:` + `sources:` |
...
| Rebuttal cycles (reviews verbatim, concern matrix, experiment ledger, drafts) | `metds/rebuttal/<cycle>/` |
| Paper sources (LaTeX, project context, outline, figures) | `paper/` |
| Survey cycles (proposal, broad scan, outline, sections, survey) | `metds/survey/<slug>/` |
