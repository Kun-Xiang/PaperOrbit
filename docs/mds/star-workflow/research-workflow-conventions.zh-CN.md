...
STAR 研究工作流中每个 skill 都遵守的规则。十七个 skill——`star-proj-adopt`、`star-idea-storm`、`star-plan-coach`、`star-refs-reviewer`、`star-code-architect`、`star-env-builder`、`star-plan-decomposer`、`star-plan-executor`、`star-code-reviewer`、`star-expt-analyst`、`star-plan-reviser`、`star-flow-status`、`star-metd-summarize`、`star-rebuttal`、`star-paper-writer`、`star-paper-reviewer`、`star-survey-writer`——各有自己的工作流、写入边界和质检表。它们共用的部分只在这里存一份。
...
**永不提交的 skill**——git 只读使用（`status` / `diff` / `log`）：`star-flow-status`、`star-refs-reviewer`、`star-expt-analyst`、`star-metd-summarize`、`star-paper-reviewer`。
...
| `star-rebuttal` | 会话结束时提供一次 | 仅本次会话创建或编辑过的 `metds/rebuttal/<cycle>/` |
| `star-paper-writer` | 会话结束时提供一次 | 仅本次会话触碰过的 `paper/**` 文件 |
| `star-survey-writer` | 会话结束时提供一次 | 仅本次会话创建或编辑过的 `metds/survey/<slug>/` |
...
| 结果台账 | `star-expt-analyst aggregate` | `metds/results.md` | `generated:` |
| 方法文档 | `star-metd-summarize` | `metds/{overview,framework,dataset,training,evaluation}.md` | `generated:`、`sources:` |
| Rebuttal | `star-rebuttal` | `metds/rebuttal/<cycle>/`——`reviews_raw.md`、`concern_matrix.md`、`experiment_ledger.md`、`rebuttal.md`、`resubmission_plan.md` | matrix 的 `stage:` 映射 + `updated:`；rebuttal 的 `drafted:` |
| 论文 | `star-paper-writer` | `paper/project_context.md`、`outline.md`、`main.tex`、`sections/`、`REVISION_LOG.md` | `project_context.md` 的 `updated:` + `sources:` |
| 论文评审 | `star-paper-reviewer` | `wkdrs/reviews/paper_<scope>_<日期>.md` | 文件名里的日期 |
| 综述 | `star-survey-writer` | `metds/survey/<slug>/`——`proposal.md`、`broad_scan.md`、`outline.md`、`sections/`、`survey.md` | proposal 的 `stage:` 映射；survey 的 `generated:` + `sources:` |
...
| 计划、笔记、方法文档 | `metds/` |
| Rebuttal 周期（评审逐字稿、concern matrix、experiment ledger、草稿） | `metds/rebuttal/<cycle>/` |
| 论文源（LaTeX、project context、outline、图） | `paper/` |
| 综述周期（方案、广度扫描、大纲、章节、综述） | `metds/survey/<slug>/` |
