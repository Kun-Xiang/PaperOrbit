...
STAR provides seventeen connected research workflow skills that turn a vague research interest into a defensible topic backed by a landscape scan, a traceable plan, a related-work base with a verified bibliography, a codebase with a recorded architecture, a verified runtime environment, executable tasks, an implementation backed by verification records, code audited against the project's conventions, results audited against what the plan expected, plans that absorb execution results, method documents compiled back out of those plans, a manuscript compiled from those documents with every claim anchored, the draft audited against the repository's own evidence, an evidence-grounded rebuttal when the reviews come back, and a standalone citation-verified survey of the field on demand:
...
vague research interest
  ...
  → star-paper-writer: compile the method documents, bibliography, and results into the manuscript
  → star-paper-reviewer: audit the draft — personas, claim audit, methodology checks
  → star-rebuttal: triage the paper's reviews, route the evidence, draft the rebuttal
  → star-survey-writer: funnel the field into a standalone survey (side track, once refs exist)

  ⌾ star-flow-status: reads all of the above at any point —
    where things stand, what is owed, and the one next action
```

The list reads as one pass, but the workflow is not linear: `star-proj-adopt` runs only when an existing project is being adopted (a project that started from the template never needs it), `star-idea-storm` runs only while the topic is still open (skip it when one is already chosen), `star-paper-writer` joins once the method documents and results ledger exist, `star-rebuttal` runs only once a submission's reviews are in hand, `star-survey-writer` is an independent side track available whenever a survey is wanted, `star-code-architect` and `star-env-builder` only run on the first pass, while `star-plan-executor` through `star-plan-reviser` is a loop you re-enter for each leaf sub-plan — `star-flow-status` names the next leaf each time round, and the audits route what they find back into the plans:
...
## 16. `$star-paper-writer`: compile the artifacts into the manuscript

### When to use it

- The method documents, bibliography, and results ledger exist, and the paper needs drafting.
- A section needs writing, rewriting, tightening, or compressing — with every claim still anchored.
- A review report or a rebuttal's revision list needs applying to the manuscript, item by item.
- You are heading to submission and want the mechanical checks (pages, references, fonts, anonymization) run for you.

### How to invoke it

```text
$star-paper-writer                 # resume the pipeline at its first unfinished stage
$star-paper-writer evaluation      # draft one section (enforced order applies)
$star-paper-writer context | outline | integrate | compress | precheck
$star-paper-writer revise          # apply a named findings source, item by item
```

### What it does

1. **Project context, compiled not interviewed**: the identity sentence, contributions-as-claims (each with its `results.md` anchor or an explicit not-yet-supported mark), locked decisions, and positioning are read out of the idea file, the root plan, and `overview.md`; only the venue, page budget, and voice overrides are asked. The `sources:` map records each input's state for exact staleness detection;
2. **Architecture**: an outline whose every section carries a page budget, a key claim with anchors, and its figures — and whose claim–evidence map proves every introduction promise has an evaluation subsection behind it;
3. **Sections in the enforced order** — Draft-0 introduction (a disposable framing scaffold) → evaluation under its guardrails → method → background → related work rewritten from `related_work.md` → the final introduction from scratch → abstract — topic sentences first, then prose;
4. **The claim ledger on every edit**: numbers must match a `results.md` row, `\cite` keys must exist in `reference.bib`, method statements trace to the method documents with not-yet-verified marks propagated, and anything unanchored becomes a visible placeholder with a route;
5. **Style gates that actually run**: the mechanical grep gate with its counts pasted, a semantic gate plus fresh-eyes red team before a section closes, the seven compression operations with before/after counts, and the venue's pre-submission mechanical checklist.

### Main outputs

```text
paper/
├── project_context.md   # identity, claims with anchors, venue, sources: map
├── outline.md           # section table, claim–evidence map, figure plan
├── main.tex + sections/ # the manuscript, drafted in the enforced order
├── figures/             # copies from wkdrs/<run>/analysis/, provenance noted
└── REVISION_LOG.md      # append-only record of revise passes
```

### The fabrication boundary

The writer writes prose, never facts: a missing number routes to `$star-expt-analyst`, a missing citation to `$star-refs-reviewer`, a missing method detail to `$star-metd-summarize`'s gap list — and the draft carries `[RESULT NEEDED]`-style placeholders until the artifact exists. Results the ledger excluded as invalid or inconclusive never enter the paper in any form.

### Practical guidance

- Run `$star-metd-summarize`, `$star-expt-analyst aggregate`, and `$star-refs-reviewer synthesize` first — the writer is only as good as the artifacts it compiles.
- Accept the Draft-0 introduction ritual: it is the cheapest insurance against an evaluation that drifts from the story.
- Treat `precheck` as part of submission, not polish — a non-embedded font found on deadline day is a bad hour.

See the complete definition in [`star-paper-writer/SKILL.md`](../../../.agents/skills/star-paper-writer/SKILL.md).

## 17. `$star-paper-reviewer`: audit the draft before submission

### When to use it

- The draft exists and you want to know how reviewers will react — before they do.
- You want every number in the paper checked against the repository's own evidence.
- You want the methodology objections (seeds, matched baselines, leakage, overclaims) predicted while they are still cheap to fix.
- A revision landed and you want a fresh read plus a re-audit.

### How to invoke it

```text
$star-paper-reviewer               # review paper/ (personas + claim audit + checklist)
$star-paper-reviewer audit         # the paper-vs-repo claim audit alone
$star-paper-reviewer quick         # single-persona fast read
$star-paper-reviewer <path>        # any tex/markdown file or directory
```

### What it does

1. **Three personas, independently**: methods-and-evidence, novelty-and-positioning skeptic, and open-minded big-picture each fill a venue-style form (strengths, weaknesses, questions, 1–10 scores with confidence), refine it once, and a meta-review merges them with weighted scores — spread shown, tensions surfaced rather than averaged;
2. **The claim audit no external reviewer can run**: every empirical claim is traced to `metds/results.md` and the run's analysis reports and classified CONFIRMED / PARTIAL / MISSING / MISMATCH / NOT_AUDITABLE, with the evidence pointer quoted — a MISMATCH quotes both sides, and an excluded-row number is a MISMATCH by definition;
3. **A methodology checklist** of the objections reviewers actually raise, each finding citing the draft line and the evidence behind it;
4. One persisted report under `wkdrs/reviews/paper_<scope>_<date>.md`, findings ranked CRITICAL / MAJOR / MINOR with routes.

### The read-only boundary

The report is the only write. Prose fixes route to `$star-paper-writer revise` (the report is a legal findings source for it), missing evidence to the plan loop, stale statements to `$star-metd-summarize`. Nothing is edited, nothing is run, and a claim that would need an experiment to check is `NOT_AUDITABLE` with the run named.

### Practical guidance

- Run it after each writer milestone, not only at the end — a MISMATCH caught at the outline stage costs a sentence; at camera-ready it costs a scramble.
- `audit` mode is cheap enough to run on a timer once a draft exists.
- Disagreement between personas is signal: a 4/7 Overall split usually marks the paragraph to rewrite.

See the complete definition in [`star-paper-reviewer/SKILL.md`](../../../.agents/skills/star-paper-reviewer/SKILL.md).

## 18. `$star-rebuttal`: triage reviews and draft the rebuttal

### When to use it

- Reviews came back and you need to decide, fast, what is worth answering and what is worth running.
- You want the rebuttal experiments scoped and executed through the same plan loop as everything else, with their evidence landing in project files.
- The results are in and the response letter needs drafting — with every number anchored to an artifact.
- All scores sit below the borderline and you want an honest read plus a resubmission plan instead of a wasted week.

### How to invoke it

```text
$star-rebuttal                    # resume the open cycle, or start a new one
$star-rebuttal neurips2026_r1     # resume a named cycle
$star-rebuttal triage             # force a stage: triage / integrate / draft / resubmit / quality
```

A new cycle asks for the venue, round, score scale, deadline, and the reviews themselves, then saves the reviews verbatim before anything is interpreted.

### What it does

1. **Viability gate**: normalizes the venue's score scale and classifies the situation — PROMISING / BORDERLINE / LOW EXPECTED RETURN — with the strongest positive and negative signals named, behind one gated question. A low-return verdict produces a resubmission roadmap rather than a full pipeline run;
2. **Triage**: splits every review into atomic concerns, diagnoses the underlying doubt behind each (with intent-diagnosis cards where the wording is genuinely ambiguous), and grades severity, sharedness, decision impact, and resolution confidence;
3. **Evidence first**: maps every concern to what the project already holds — `metds/results.md`, `wkdrs/<run>/EXPT_ANALYSIS_*.md`, refs notes, the manuscript — before commissioning anything new;
4. **Experiment plan**: ranks the genuinely missing evidence P0–P3 (minimum viable protocol, cost, and what a negative result would mean, per row) and routes the confirmed units to `$star-plan-decomposer` as proposed leaves — execution and scoring stay with `$star-plan-executor` and `$star-expt-analyst`, whose outputs flow back into the cycle's ledger;
5. **Integrate & draft**: validates returned results against the concerns they claim to answer, then drafts the rebuttal with Direct Answer → Evidence → Revision — every evidence sentence anchored to a project artifact, a verified manuscript location, or a visible placeholder. `quality` audits an existing draft against the checklist.

### Main outputs

```text
metds/rebuttal/<venue>_<round>/
├── reviews_raw.md          # the reviews, verbatim, append-only
├── concern_matrix.md       # atomic concerns, diagnosis, evidence map, decisions
├── experiment_ledger.md    # P0–P3 plan plus leaf / run / report pointers
└── rebuttal.md             # or resubmission_plan.md in resubmit mode
```

### The evidence boundary

The skill runs no experiments and writes no plan files: rebuttal experiments become ordinary leaves via `$star-plan-decomposer`, cross the STOP line inside `$star-plan-executor` like any training run, and return as `$star-expt-analyst` reports. A claim with no artifact behind it enters the draft only as a visible `[RESULT NEEDED]`-style placeholder — never as prose that sounds finished. Manufactured reviewer praise, silent claim upgrades, and hidden negative results are all forbidden outright.

### Practical guidance

- Run it the day the reviews arrive: the viability gate costs minutes and is cheapest before any work is sunk.
- One cycle per venue round; a later revise-and-resubmit round gets its own cycle directory and ledger.
- The drafted manuscript-revision list doubles as the work order for the camera-ready or the next submission.

See the complete definition in [`star-rebuttal/SKILL.md`](../../../.agents/skills/star-rebuttal/SKILL.md).

## 19. `$star-survey-writer`: funnel a field into a survey

### When to use it

- You want a standalone survey of a field — for the community, a thesis chapter, or your own orientation — not just the paper's related-work section.
- The refs machinery exists and you want the wide-angle pass: 100+ papers scanned, the right 15–25 read.
- A survey cycle exists and the refs base has grown — time to refresh.

### How to invoke it

```text
$star-survey-writer open-vocabulary segmentation    # new cycle on a topic
$star-survey-writer <slug>                          # resume a cycle
$star-survey-writer outline | draft <section> | refine
```

### What it does

1. **Proposal (gate)**: a small probe, then keywords, sub-directions, selection criteria, target scale, and reader profile — confirmed before any full scan;
2. **Breadth**: 100+ papers' metadata through the refs family's source policy, every payload cached before use, no PDFs;
3. **Selection (gate)**: mandatory rows first — recent surveys, seminal works, last year's top-venue frontier — then ranked fill to a 15–25 pool;
4. **Depth, delegated**: each pool paper routes to `$star-refs-reviewer`, whose notes and verified `reference.bib` are the only sources the draft may characterize from or cite;
5. **Outline (gate)**: learned from how the scanned surveys structure the field, with per-section paper assignments and a coverage check;
6. **Draft and refine**: sections bounded by the notes' depth, a reviewer–refiner pass, intro and conclusion written last; `survey.md` records the refs base's state for exact staleness detection.

### The provenance boundary

The survey writes nothing under `metds/refs/**` and fetches nothing beyond scan metadata: records, notes, and bib entries stay under `$star-refs-reviewer`'s audit trail. Every citation exists in the bib, every characterization stays within its note's depth, a scan-only paper is named but never characterized, and thin themes become stated gaps with read-next routes — never padded prose.

### Practical guidance

- The mandatory-inclusion rows are the funnel's quality floor: recent surveys calibrate the taxonomy before you invent one.
- Draft sections across sessions freely — the outline's status map is the resume point.
- `$star-paper-reviewer metds/survey/<slug>/survey.md` gives the assembled survey a full persona review.

See the complete definition in [`star-survey-writer/SKILL.md`](../../../.agents/skills/star-survey-writer/SKILL.md).
...
### Step 9: draft the paper

```text
$star-paper-writer
```

Once the method documents, `related_work.md`, and `metds/results.md` exist, the writer compiles the project context, the outline, and the sections in the enforced order — every number anchored to the results ledger, every citation checked against `reference.bib`. Before submission, `$star-paper-reviewer` audits the draft — three reviewer personas plus a claim audit against the results ledger. When the reviews come back, `$star-rebuttal` takes over, and its manuscript-revision list returns here through `$star-paper-writer revise`.
...
| The method documents and results ledger are ready and the manuscript needs drafting | `$star-paper-writer` |
| The draft is ready and you want it reviewed — and its numbers audited — before submission | `$star-paper-reviewer` |
| The submission's reviews came back and you need a triage, rebuttal experiments, or the response letter | `$star-rebuttal` |
| You want a standalone, citation-verified survey of a field | `$star-survey-writer` |
...
The seventeen skill directory names are:
