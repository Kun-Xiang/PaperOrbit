...
STAR 提供十七个相互衔接的研究工作流 skill，用于把一个模糊的研究兴趣逐步变成有文献扫描背书的研究选题、可追踪的计划、有可核验文献库的相关工作基座、有架构记录的代码库、经过验证的运行环境、可执行的任务、有验证记录的实现、对照规范审计过的代码、对照预期审计过的实验结果、能吸收执行结果的计划、从这些计划反向编译出的方法文档、从这些文档编译且每个论断都有锚定的稿件、对照仓库自身证据审计过的草稿、评审返回后有证据支撑的 rebuttal，以及随需的领域独立综述（引用全程可核验）：
...
模糊的研究兴趣
  ...
  → star-paper-writer：把方法文档、文献库与结果台账编译成稿件
  → star-paper-reviewer：审计草稿——人格评审、声明对账、方法论检查
  → star-rebuttal：分诊论文评审、路由证据、起草 rebuttal
  → star-survey-writer：把领域漏斗成独立综述（侧线，refs 机制就绪即可）

  ⌾ star-flow-status：随时通读上述全部产物——
    进度到哪里、还欠什么、下一步该做哪一件
```

上面的列表读起来是一条直线，但实际流程并非线性：`star-proj-adopt` 只在接入已有项目时跑（从模板起步的项目永远用不到它），`star-idea-storm` 只在选题未定时跑（选题已定就跳过），`star-paper-writer` 在方法文档与结果台账就绪后加入，`star-rebuttal` 只在某次投稿的评审到手后跑，`star-survey-writer` 是想写综述时随时可走的独立侧线，`star-code-architect` 和 `star-env-builder` 只在第一轮跑，而 `star-plan-executor` 到 `star-plan-reviser` 是一个循环，每个叶子子计划都会重新走一遍——`star-flow-status` 每轮给出下一个该跑的叶子，审计环节则把结论路由回计划本身：
...
## 16. `$star-paper-writer`：把制品编译成稿件

### 什么时候用

- 方法文档、文献库与结果台账都在了，论文该起草了。
- 某章节要写、要重写、要收紧或压缩——同时每个论断仍要有锚定。
- 一份评审报告或 rebuttal 的修订清单要逐条施加到稿件上。
- 临投稿，想让机械检查（页数、引用、字体、匿名化）替你跑一遍。

### 怎么调用

```text
$star-paper-writer                 # 从第一个未完成阶段续做
$star-paper-writer evaluation      # 起草某一章节（强制顺序仍然适用）
$star-paper-writer context | outline | integrate | compress | precheck
$star-paper-writer revise          # 逐条施加一个具名 findings 来源
```

### 它做什么

1. **Project context，编译而非访谈**：身份句、写成 claim 的贡献（各带 `results.md` 锚定或显式 not-yet-supported 标记）、锁定决策与定位，从 idea 文件、root plan 与 `overview.md` 读出；只有 venue、页预算与声音覆盖需要问。`sources:` 映射记录每个输入的状态值，用于精确过期检测；
2. **架构**：大纲给每个章节页预算、带锚定的关键 claim 与图表——claim–证据映射证明 introduction 的每个承诺背后都有 evaluation 小节；
3. **按强制顺序起草章节**——Draft-0 introduction（可弃的框架脚手架）→ 护栏下的 evaluation → method → background → 从 `related_work.md` 改写的 related work → 从零重写的最终 introduction → abstract——先 topic sentences 后正文；
4. **每次编辑跑 claim 台账**：数字必须对上 `results.md` 的行、`\cite` 键必须存在于 `reference.bib`、方法陈述回溯方法文档且 not-yet-verified 标记随行，无锚定内容变成带路由的可见占位符；
5. **实跑的风格门**：机械 grep 门贴计数、章节关闭前过语义门与新眼红队、压缩七操作报前后计数、venue 投稿前机械检查单。

### 主要产出

```text
paper/
├── project_context.md   # 身份、带锚定的 claims、venue、sources: 映射
├── outline.md           # 章节表、claim–证据映射、图表计划
├── main.tex + sections/ # 按强制顺序起草的稿件
├── figures/             # 自 wkdrs/<run>/analysis/ 拷贝，注明出处
└── REVISION_LOG.md      # revise 轮次的只增记录
```

### 造假边界

Writer 写行文，绝不造事实：缺数字路由 `$star-expt-analyst`，缺引用路由 `$star-refs-reviewer`，缺方法细节路由 `$star-metd-summarize` 的缺口清单——制品存在之前草稿带着 `[RESULT NEEDED]` 类占位符。台账排除为 invalid / inconclusive 的结果以任何形式都不入稿。

### 实用建议

- 先跑 `$star-metd-summarize`、`$star-expt-analyst aggregate` 与 `$star-refs-reviewer synthesize`——writer 的上限就是它所编译制品的质量。
- 接受 Draft-0 introduction 的仪式：它是防止 evaluation 偏离故事线的最便宜保险。
- 把 `precheck` 当投稿的一部分而不是抛光——deadline 当天发现字体没内嵌，是很糟的一小时。

完整定义见 [`star-paper-writer/SKILL_zh.md`](../../../.agents/skills/star-paper-writer/SKILL_zh.md)。

## 17. `$star-paper-reviewer`：投稿前审计草稿

### 什么时候用

- 草稿已在，想在审稿人反应之前知道他们会怎么反应。
- 想把论文里每个数字对照仓库自身证据核一遍。
- 想在还便宜的时候预判方法论质疑（seeds、对齐的 baseline、泄漏、过度声称）。
- 一轮修订落地，想要新鲜的通读加复审。

### 怎么调用

```text
$star-paper-reviewer               # 审 paper/（人格 + 声明对账 + 检查单）
$star-paper-reviewer audit         # 只跑 paper-vs-repo 声明对账
$star-paper-reviewer quick         # 单人格速评
$star-paper-reviewer <path>        # 任何 tex/markdown 文件或目录
```

### 它做什么

1. **三人格独立评审**：方法与证据、novelty 与定位质疑者、开放的大局观，各自填 venue 式评审表（优点、弱点、问题、1–10 分与置信度），各精炼一轮，meta-review 加权合并——展示离散度，把张力摆出来而不是平均掉；
2. **外部审稿者做不了的声明对账**：每条实证声明追溯 `metds/results.md` 与该 run 的分析报告，归类 CONFIRMED / PARTIAL / MISSING / MISMATCH / NOT_AUDITABLE 并引证据指针——MISMATCH 两边原文对引，被排除行的数字按定义即 MISMATCH；
3. **方法论检查单**：审稿人真实会提的质疑，每条发现引草稿行与背后证据；
4. 一份持久化报告落在 `wkdrs/reviews/paper_<scope>_<date>.md`，发现按 CRITICAL / MAJOR / MINOR 排序并带路由。

### 只读边界

报告是唯一写入。行文修正路由 `$star-paper-writer revise`（本报告是其合法 findings 来源），缺证据走计划回路，过期陈述给 `$star-metd-summarize`。不改任何东西、不跑任何东西，需要实验才能核的声明是 `NOT_AUDITABLE` 并点名那个 run。

### 实用建议

- 在 writer 的每个里程碑后跑，别只在最后跑——outline 阶段抓到的 MISMATCH 只花一句话，camera-ready 阶段抓到就是一场手忙脚乱。
- 草稿存在后，`audit` 模式便宜到可以挂定时任务。
- 人格间的分歧是信号：Overall 4 分对 7 分的分裂通常正标出该重写的段落。

完整定义见 [`star-paper-reviewer/SKILL_zh.md`](../../../.agents/skills/star-paper-reviewer/SKILL_zh.md)。

## 18. `$star-rebuttal`：分诊评审并起草 rebuttal

### 什么时候用

- 评审回来了，你需要快速决定哪些值得回答、哪些值得跑实验。
- 你想让 rebuttal 实验走和其他工作相同的计划回路，证据自动落进项目文件。
- 结果已经到手，回应信要起草——每个数字都要有锚定。
- 所有分数都低于 borderline，你想要一个诚实的判断加一份转投计划，而不是浪费一周。

### 怎么调用

```text
$star-rebuttal                    # 续做未完成的周期，或新建
$star-rebuttal neurips2026_r1     # 续做指定周期
$star-rebuttal triage             # 强制进入某阶段：triage / integrate / draft / resubmit / quality
```

新周期会询问 venue、轮次、分制、DDL 与评审全文，然后先逐字保存评审，再开始任何解读。

### 它做什么

1. **可行性门**：归一化 venue 分制，判定局面——PROMISING / BORDERLINE / LOW EXPECTED RETURN——点名最强的正负信号，经一个门控问题确认。低回报的判定产出转投路线图，而不是硬跑完整流水线；
2. **分诊**：把每份评审拆成原子意见，诊断每条背后的底层疑虑（措辞真正含混的用意图诊断卡呈现），并给出严重度、共享度、决策影响与解决置信度；
3. **证据先行**：先把每条意见映射到项目已有的东西——`metds/results.md`、`wkdrs/<run>/EXPT_ANALYSIS_*.md`、refs 笔记、稿件——再考虑新实验；
4. **实验计划**：把真正缺失的证据排成 P0–P3（每行带 minimum viable protocol、成本、阴性结果的含义），确认的单元路由给 `$star-plan-decomposer` 立为候选叶子——执行与打分归 `$star-plan-executor` 和 `$star-expt-analyst`，其产出回流到周期的台账；
5. **整合与起草**：校验返回结果是否回答其声称的疑虑，然后按 Direct Answer → Evidence → Revision 起草 rebuttal——每句证据锚定项目制品、经核实的稿件位置或可见占位符。`quality` 按检查单审计既有草稿。

### 主要产出

```text
metds/rebuttal/<venue>_<round>/
├── reviews_raw.md          # 评审逐字稿，只增不改
├── concern_matrix.md       # 原子意见、诊断、证据映射、决定
├── experiment_ledger.md    # P0–P3 计划 + 叶子/run/报告指针
└── rebuttal.md             # resubmit 模式则为 resubmission_plan.md
```

### 证据边界

本 skill 不跑实验、不写计划文件：rebuttal 实验经 `$star-plan-decomposer` 变成普通叶子，在 `$star-plan-executor` 里像任何训练一样越过 STOP 线，再以 `$star-expt-analyst` 报告的形式回来。没有制品支撑的论断只能以可见的 `[RESULT NEEDED]` 类占位符进入草稿——绝不以听起来已完成的行文出现。编造评审赞扬、悄悄拔高论断、隐藏阴性结果都被直接禁止。

### 实用建议

- 评审到的当天就跑：可行性门只花几分钟，在投入任何工作之前最便宜。
- 一个 venue 轮次一个周期；之后的 revise-and-resubmit 轮次开自己的周期目录与台账。
- 起草出的稿件修订清单，就是 camera-ready 或下一次投稿的工单。

完整定义见 [`star-rebuttal/SKILL_zh.md`](../../../.agents/skills/star-rebuttal/SKILL_zh.md)。

## 19. `$star-survey-writer`：把一个领域漏斗成综述

### 什么时候用

- 想要一份领域的独立综述——给社区、学位论文章节或自己入门用——而不只是论文的 related-work 节。
- refs 机制已就绪，想要广角一遍：扫 100+ 篇，读对那 15–25 篇。
- 综述周期已存在而 refs 基座长大了——该刷新了。

### 怎么调用

```text
$star-survey-writer open-vocabulary segmentation    # 以主题开新周期
$star-survey-writer <slug>                          # 续做周期
$star-survey-writer outline | draft <section> | refine
```

### 它做什么

1. **方案（门）**：小探针后定关键词、细分方向、筛选标准、目标规模与读者画像——任何全量扫描之前确认；
2. **广度**：经 refs 家族 source policy 扫 100+ 篇元数据，payload 先缓存后使用，不下 PDF；
3. **筛选（门）**：强制行优先——近年综述、开山作、最近一年顶会前沿——再按名次补足 15–25 池；
4. **深度，委托**：池内每篇路由 `$star-refs-reviewer`，其笔记与已验证 `reference.bib` 是草稿唯一可特征化、可引用的来源；
5. **大纲（门）**：从被扫描综述如何组织领域中学来，带每节论文指派与覆盖检查；
6. **起草与 refine**：章节以笔记 depth 为界，一轮 reviewer–refiner，intro 与 conclusion 最后写；`survey.md` 记录 refs 基座状态用于精确过期检测。

### 溯源边界

综述不写 `metds/refs/**` 下任何东西、不抓扫描元数据之外的任何东西：记录、笔记与 bib 条目始终在 `$star-refs-reviewer` 的审计链下。每条引用存在于 bib、每句特征化不超其笔记 depth、只有扫描记录的论文可点名不可特征化、薄主题写成带待读路由的明示缺口——绝不注水。

### 实用建议

- 强制入选行是漏斗的质量下限：先让近年综述校准分类学，再谈自创。
- 章节尽管跨会话起草——大纲的状态映射就是续做起点。
- `$star-paper-reviewer metds/survey/<slug>/survey.md` 可给拼合综述做全套人格评审。

完整定义见 [`star-survey-writer/SKILL_zh.md`](../../../.agents/skills/star-survey-writer/SKILL_zh.md)。
...
方法文档、`related_work.md` 与 `metds/results.md` 就绪后，writer 依次编译 project context、大纲与各章节（强制顺序）——每个数字锚定结果台账、每条引用对照 `reference.bib` 核验。投稿前先由 `$star-paper-reviewer` 审计草稿——三人格评审加对照结果台账的声明对账。评审回来后交给 `$star-rebuttal`，其稿件修订清单再经 `$star-paper-writer revise` 回到这里。
...
| 方法文档与结果台账已就绪，稿件要起草 | `$star-paper-writer` |
| 草稿已就绪，想在投稿前得到评审并核对数字 | `$star-paper-reviewer` |
| 投稿的评审回来了，需要分诊、rebuttal 实验或回应信 | `$star-rebuttal` |
| 想要一份引用可核验的领域独立综述 | `$star-survey-writer` |
...
- **可以挂定时任务**：`$star-flow-status`（只读、无提问）；`$star-paper-reviewer`（只读，唯一写入是它自己的报告）；带明确目标的 `$star-expt-analyst <叶子 | run 目录>`，以及 `$star-expt-analyst watch <叶子>`（只在聊天里）；重编译的 `$star-metd-summarize`——来源没动的文档原样不动，实质性覆写会停在变更清单的提问上，不会直接盖掉。
...
- **需要你在场**：`$star-idea-storm`、`$star-plan-coach`、`$star-plan-decomposer`、`$star-code-architect`、`$star-env-builder`、`$star-plan-executor`、`$star-code-reviewer`、`$star-plan-reviser`、`$star-rebuttal`、`$star-paper-writer`、`$star-survey-writer`——它们的提问与门就是设计本身；用脚本替它们答"是"，恰恰毁掉了这些门要保护的审计链。
